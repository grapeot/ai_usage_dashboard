...
Note: the preview uses PIL's default bitmap font and saturated RGB colors. The actual E1002 panel runs in 1-bit black-and-white mode, so colored bars render as solid black on hardware — see "Panel Color Behavior" below. Treat preview colors as a layout check, not a hardware prediction.
...
## Panel Color Behavior (1-bit thresholding)

The ED103TC2 controller supports 16-level grayscale, but this sketch runs in **1-bit black-and-white mode** (it never calls `EPaper::initGrayMode`). Every fill color is thresholded before it reaches the panel:

1. The 16-bit color is packed into an 8-bit RGB332 byte in the sprite buffer (Seeed_GFX `Sprite.cpp`: `(color & 0xE000)>>8 | (color & 0x0700)>>6 | (color & 0x0018)>>3`).
2. The 1-bit display path (`tconDisplayArea1bpp`) maps bytes `< 128` to black and `>= 128` to white.

Consequences verified on hardware (2026-09-25):

- Every solid color in the palette (green, yellow, red, blue, cyan) packs under 128 and renders as **solid black** on the panel. Provider identity is carried by labels and by the white-plus-black-pattern fills in the stacked chart (stripes, dots, reverse stripes).
- Mid gray `0x8210` packs to 138 (>= 128) and renders as **white**: the first Cursor quota bars shipped with this fill and a 100%-full bar looked completely empty on the panel while the simulator preview showed it filled. They were switched to black.
- Rule: only use black fills or white + black pattern fills for anything that must be visible. Do not use gray fills.
- `eink_simulator.py` renders saturated RGB and does not simulate this thresholding. If a color choice is in question, verify on the device.
...
- One horizontal usage bar per quota window, stacked vertically. Each bar is 14px tall with a filled colored portion (GLM=green, Ollama=cyan, Codex=yellow, Claude=red, Grok=black, Cursor=black) and a black outline. On the 1-bit panel every solid color packs under the threshold and renders black; a mid-gray fill would pack over it and render white (a full bar looking empty), so provider identity is carried by the label text, not the fill. Below each bar, one line of text: `<Provider> <label>, reset in Xd Yh` (or `reset in X.Yh` when less than a day remains). The reset countdown is computed from the NTP-synchronized local clock.
- Labels are compact: `5h` for 5-hour windows, `7d` for weekly windows, `Monthly`-style window names otherwise
- Provider order: GLM, Ollama, Codex, Claude, Antigravity, Grok, Cursor
